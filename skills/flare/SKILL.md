...
- `references/platform-sizes.md`: Platform and deliverable-specific image sizes, aspect ratios, safe areas, and compliance guidance for social, ads, ecommerce, app stores, web sharing, video, documents, print, publishing, packaging, and merchandise.
...
1. Resolve the target environment, MCP project id, browser project URL, and live `agentSessionId` before visible edits. Prefer MCP-returned `projectId` for tool calls and `projectUrl` for browser navigation. MCP project URLs may include an `agentSession` query parameter; open them exactly as returned so the browser can bind its live selection/viewport to the MCP operation. Pass the returned `agentSessionId` to later live-context, placement, and `autoInsert` calls when available. MCP responses may also include `projectRouteId`, the prefix-free browser route id. If the target is unclear, follow the Project Selection Pattern below before opening a browser tab or editing.
...
This section is complete for the hot path. Do not open or read any `references/*` file before generation or upload unless the request mentions a target platform/channel, deliverable type, exact size/export, leaves the fast path, or an MCP call fails. When the user names a platform or deliverable such as Xiaohongshu/小红书, Instagram, TikTok, Amazon, Etsy, YouTube, App Store, Open Graph, podcast cover, A4 poster, business card, or T-shirt print, read `references/platform-sizes.md` before writing the image prompt.

1. Start the agent/client image generation immediately, after choosing the platform or deliverable aspect ratio when the user named a target platform, placement, or physical/digital deliverable.
2. In parallel with image generation, resolve the target project and open/focus its `projectUrl` directly in the browser when browser control is available. If the response includes `agentSessionId`, keep it for later MCP calls. In Codex desktop, use the in-app browser control path, not Chrome DevTools or the user's external Chrome, and keep it visible through upload/insertion. Use `check_client_setup` and Project Selection Pattern only as needed for setup or target resolution; do not read workflow/upload references first.
...
4. When the local image file is ready, use the exact path returned by the image generation tool. Do not search recent generated-image folders, infer the newest file, or copy the file into the workspace or a `flare-assets` folder. Call `create_image_upload_session` with provenance and `autoInsert: { projectId, agentSessionId }` when `agentSessionId` is available, then binary-upload that file's raw bytes to the returned `uploadUrl`. The upload response should include both `assetId` and `autoInsert`.
...
4. If there is exactly one active project, use it by default and open its `projectUrl` directly before visible edits unless the user explicitly asked to choose or the surrounding context points to another project. Keep its `selectedAgentSessionId` for later calls when provided.
5. If there are multiple active projects, do not infer the first result. Present up to five choices with name, id, updated time, and `projectUrl`, then wait for the user to reply with a number, name, or project id. Keep each choice's `agentSessionId` internally. After the user chooses, open that exact `projectUrl` directly and keep the choice's `agentSessionId`.
...
- When an MCP response includes `projectUrl`, use it directly for browser navigation. It should already be the prefix-free editor route and may include `?agentSession=...` for live browser binding.
...
- **Plain "generate an image/photo" requests default to the Fast Path**: if the user asks to generate a picture/photo/illustration for Flare and does not explicitly say to use Flare backend generation, start agent/client image generation first and resolve/open the Flare project in parallel. If the user names a target platform, placement, marketplace, ad channel, app store asset, web sharing format, print document, publishing cover, packaging, or merchandise deliverable, read `references/platform-sizes.md` first and use that deliverable's aspect ratio, safe-area, and compliance guidance in the generation prompt. Preserve any `agentSessionId` returned with the target project and pass it to later `autoInsert` calls. Do not satisfy a generation request by reusing an existing Flare asset just because the prompt, name, or visual content looks similar. Do not search existing assets or inspect existing canvas images for ordinary generation. Existing assets may be reused only when the user explicitly asks to reuse/import an existing asset, or when recovering from a partially successful upload of the exact file generated in the current operation. If the result is a local file, use the exact image generation output path; do not search for recently generated files or copy the file into the workspace before upload. Call `create_image_upload_session` with provenance (`sourceClient`, `generationPrompt`, `generationModel`, `generationTool` when known) and `autoInsert: { projectId, agentSessionId }` when available, then binary-upload it to Assets. Stop when the upload response confirms `autoInsert`; call `insert_asset_image` only when autoInsert is unavailable or failed. If the current client only exposes `get_image_upload_endpoint`, use its returned `uploadToken` and then call `insert_asset_image`.
- **The agent generated or obtained an image**: prefer a local file or public HTTPS URL. For local files, create an MCP upload session with `autoInsert` when a target project is known, include `agentSessionId` when the project URL response provided one, then upload raw bytes; do not put base64 or data URLs in MCP JSON. If the image currently exists as a data URL or base64 string, save it to a local image file first, then upload that file. Use `insert_asset_image` only when `autoInsert` is unavailable or failed. Use `insert_agent_generated_image` only for public URLs. Do not call `create_generation_job`. Treat these assets as generated media entering Flare through MCP binary upload, not ordinary user uploads. When no exact `x/y` is required, omit `width` and `height` so Flare preserves the original image dimensions, and use smart placement or explicit visible coordinates so the inserted layer appears in the current/default viewport.
...
- If the user names a target platform, channel, or deliverable type, read `references/platform-sizes.md` and use that platform or deliverable's recommended canvas size for the root frame unless the user gave an exact size.
...
- Use the local file path returned by the generation/edit tool. Do not run shell searches to find the newest generated image, and do not copy the generated file into the workspace or a project asset folder before upload unless the user explicitly asked for a local backup.
