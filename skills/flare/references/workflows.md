...
3. Identify the MCP `projectId` from an explicit id, the selected project, or the browser URL. If reading from `/projects/{routeId}`, add `proj_` unless already present. When operating in a browser-capable client, resolve the direct `projectUrl` before opening or changing browser tabs, then open that exact URL before visible edits; do not navigate through the app root or project list page. Browser URLs use the route id without one leading `proj_` prefix.
