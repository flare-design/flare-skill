...
1. Start the available client-side image generation or retrieval capability immediately to create or obtain a fresh bitmap. If the user names a target platform, placement, marketplace, ad channel, app store asset, web sharing format, print document, publishing cover, packaging, or merchandise deliverable, read `platform-sizes.md` first and use its recommended aspect ratio, safe-area, and compliance guidance in the generation prompt.
...
4. Keep or convert the result into a local image file. When the generation/edit tool returns a local path, use that exact path for upload; do not search generated-image directories for the newest file and do not copy it into the workspace first. If the client produced a data URL or base64 string, decode it and write it to disk before touching Flare MCP.
5. Call `create_image_upload_session` with provenance and `autoInsert: { projectId }`, then binary-upload the local file into Assets with the returned `uploadUrl` and `uploadToken`. Use `sourceClient` for the calling agent/client (`codex`, `claude`, `chatgpt`, `cursor`), `generationPrompt` for the prompt, `generationModel` for the actual image model, and `generationTool` for the generation surface when known. If that tool is not exposed in the current client's cached tool list, call `get_image_upload_endpoint` and use its returned generic `uploadToken`. The upload response returns `assetId`, an `asset` object, and `autoInsert` when available.
6. End after the upload response confirms `autoInsert`. Do not call `insert_asset_image`, `get_canvas_snapshot`, refresh the browser, center the viewport, or visually verify unless the user explicitly asks, an MCP call failed, auth is missing, or the user says the result is not visible. Use `insert_asset_image` only when `autoInsert` is unavailable or failed. Omit `width` and `height` by default so Flare preserves the generated bitmap's original dimensions.
...
6. Generate a fresh revised bitmap with the agent/client image generation or image-edit capability. This is mandatory for annotation revision; after annotation context is read, the next substantive action should be image generation or image edit. Do not use canvas shapes/text, ordinary layer edits, existing asset reuse, or deterministic local image-processing scripts such as Pillow/canvas compositing as a substitute for generation. If no image generation/edit capability is available, report that limitation instead of faking the edit. Do not call `create_generation_job` unless the user explicitly asks for Flare backend generation.
7. Keep the revised output as a local file. When the generation/edit tool returns a local path, use that exact path for upload; do not search generated-image directories for the newest file and do not copy it into the workspace first. If the generation result is a data URL or base64 string, decode it to a local image file first.
8. Call `create_image_upload_session` with provenance and `autoInsert` from `suggestedAutoInsert`. Include the original prompt if known and mention the annotation session id in `generationNotes` when useful.
9. Binary-upload raw file bytes, then stop after the upload response confirms `autoInsert`. Use `insert_asset_image` with `suggestedPlacement` only when `autoInsert` is unavailable or failed. Do not invent a smaller display size unless the user asked for resizing.
