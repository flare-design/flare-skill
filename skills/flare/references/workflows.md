...
3. Identify `projectId` from the project URL, an explicit user-provided id, or the project selected through the Project Selection Pattern in `SKILL.md`. When operating in a browser-capable client, resolve the direct `projectUrl` before opening or changing browser tabs, then open that exact URL before visible edits; do not navigate through the app root or project list page.
...
1. Identify `projectId` from the project URL, an explicit user-provided id, or the project selected through the Project Selection Pattern in `SKILL.md`. When operating in a browser-capable client, resolve the direct `projectUrl` before opening or changing browser tabs, then open that exact URL before visible edits; do not navigate through the app root or project list page.
