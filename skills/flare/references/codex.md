...
- Browser `projectUrl` route ids omit one leading `proj_` prefix. MCP tool `projectId` arguments keep the full `proj_...` id. When reading `/projects/{routeId}` from the browser, add `proj_` back before MCP calls unless it is already present.
